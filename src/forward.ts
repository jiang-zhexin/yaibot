...
forward.on("msg:forward_origin", async (c) => {
    ...
    let result = await db.update(inlines).set(inline).where(eq(inlines.mark, true)).limit(1).returning({ id: inlines.id })
    if (result.length === 0) {
        result = await db.insert(inlines).values(inline).returning({ id: inlines.id })
    }
...
function getUserInfo(forward_origin: MessageOriginUser | MessageOriginHiddenUser | MessageOriginChat | MessageOriginChannel): string {
    switch (forward_origin.type) {
        case "hidden_user":
            return forward_origin.sender_user_name
        case "user":
            return forward_origin.sender_user.first_name + (forward_origin.sender_user.last_name || "")
        case "chat":
            switch (forward_origin.sender_chat.type) {
                ...
                case "private":
                    return forward_origin.sender_chat.first_name + (forward_origin.sender_chat.last_name || "")
            }
        case "channel":
